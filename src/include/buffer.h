...
public:
  ...
  public:
    ...
    void claim(list& bl, bool clone_nonshareable=true);
    void claim_append(list& bl, bool clone_nonshareable=true);
    void claim_prepend(list& bl, bool clone_nonshareable=true);

    // clone non-shareable buffers (make shareable)
    void clone_nonshareable() {
      std::list<buffer::ptr>::iterator pb;
      for (pb = _buffers.begin(); pb != _buffers.end(); ++pb) {
        (void) pb->clone_nonshareable();
      }
    }
