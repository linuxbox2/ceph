 ...
 */
BIIndexType bi_type(const string& s)
{
  int ret = bi_entry_type(s);
  if (ret < 0) {
...
int rgw_bucket_prepare_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if (header.resharding_in_logrecord())
    cls_cxx_subop_version(hctx, &entry.sub_ver); // get ver.subop of prepare
  ...
  if (header.resharding_in_logrecord()) {
    // write the log with newest gen
    return reshard_log_index_operation(hctx, op.key, idx, header.get_gen(),
                                       header.ver, entry.sub_ver, op.op);
  }
                               ...
                               const cls_rgw_obj_key& key)
{
  ...
  if (header.resharding_in_logrecord()) {
    cls_cxx_subop_version(hctx, &entry.sub_ver); // every key entry has its own sub_ver
    int rc = reshard_log_index_operation(hctx, key, idx, header.get_gen(),
                                     header.ver, entry.sub_ver, CLS_RGW_OP_DEL);
    if (rc < 0) {
...
int rgw_bucket_complete_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if (header.resharding_in_logrecord()) {
    rc = reshard_log_index_operation(hctx, op.key, idx, header.get_gen(),
                                     header.ver, entry.sub_ver, op.op);
    if (rc < 0) {
      CLS_LOG_BITX(bitx_inst, 1,
...
static int rgw_bi_get_vals_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  rgw_cls_bi_get_vals_ret op_ret;
...
/* put the index entry generated in processing reshard log to new shard */
static int rgw_bi_process_log_put_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  // decode request
  rgw_cls_bi_process_log_put_op op;
  auto iter = in->cbegin();
  try {
    decode(op, iter);
  } catch (ceph::buffer::error& err) {
  ...
  if (ori_entry.exists) {
    ...
    int r = cls_cxx_map_set_val(hctx, ori_entry.bi_entry.idx, &ori_entry.bi_entry.data);
    if (r < 0) {
      CLS_LOG(0, "ERROR: %s(): cls_cxx_map_set_val() returned r=%d", __func__, r);
    }
    ...
    int r = cls_cxx_map_remove_key(hctx, ori_entry.idx);
    if (r < 0) {
...
int reshard_log_record_decode(bufferlist& bl, rgw_reshard_log_entry& e)
{
  auto iter = bl.cbegin();
  try {
    decode(e, iter);
  } catch (ceph::buffer::error& err) {
    CLS_LOG(0, "ERROR: failed to decode rgw_reshard_log_entry");
    return -EIO;
  }
  return 0;
}
			   ...
			   uint32_t max, list<rgw_reshard_log_entry>& entries, bool *truncated)
{
  ...
  for (iter = keys.begin(); iter != keys.end(); ++iter) {
    ...
    ret = reshard_log_record_decode(iter->second, e);
    if (ret < 0)
      return ret;
...
static int rgw_reshard_log_list_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  // decode request
  cls_rgw_reshard_log_list_op op;
  auto iter = in->cbegin();
  try {
    decode(op, iter);
  } catch (ceph::buffer::error& err) {
...
static int rgw_set_bucket_resharding(cls_method_context_t hctx, bufferlist *in,  bufferlist *out)
{
  ...
  header.new_instance.set_status(op.entry.reshard_status, op.entry.gen);
