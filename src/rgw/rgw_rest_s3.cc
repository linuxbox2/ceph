					      ...
					      off_t bl_len)
{
  ...
  if (! op_ret) {
	...
	if (strncmp(name, RGW_ATTR_META_PREFIX,
...
int RGWHandler_REST_S3::init(RGWRados *store, struct req_state *s,
				 RGWClientIO *cio)
{
  dout(10) << "s->object=" << (!s->object.empty() ? s->object
			       : rgw_obj_key("<NULL>"))
	   << " s->bucket=" << (!s->bucket_name_str.empty() ?
				s->bucket_name_str : "<NULL>")
	   << dendl;
  ...
  if (s->copy_source) {
    ret =
      RGWCopyObj::parse_copy_location(s->copy_source, s->src_bucket_name,
				      s->src_object);
    if (!ret) {
...
int RGW_Auth_S3_Keystone_ValidateToken::validate_s3token(
  ...
  ldout(cct, 5) << "s3 keystone: validated token: "
		<< response.token.tenant.name << ":" << response.user.name
		<< " expires: " << response.token.expires << dendl;
  return 0;
}
...
int RGW_Auth_S3::authorize(RGWRados *store, struct req_state *s)
{
  ...
  /* try external authenticators first */
...
{
  int ret = RGWHandler_REST_S3::init_from_header(s, RGW_FORMAT_XML, false);
  if (ret < 0)
    return NULL;

  if (s->bucket_name_str.empty())
    return new RGWHandler_REST_Service_S3;

  if (s->object.empty())
    return new RGWHandler_REST_Bucket_S3;

  return new RGWHandler_REST_Obj_S3;
}
