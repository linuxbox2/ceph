				  ...
				  map<string, bufferlist>& attrs)
{
  ...
  for (iter = attrs.lower_bound(RGW_ATTR_META_PREFIX); iter != attrs.end();
       ++iter) {
...
static void dump_container_metadata(struct req_state *s, RGWBucketEnt& bucket)
{
  ...
  if (s->object.empty()) {
    ...
    const size_t PREFIX_LEN = sizeof(RGW_ATTR_META_PREFIX) - 1;
    map<string, bufferlist>::iterator iter;
    for (iter = s->bucket_attrs.lower_bound(RGW_ATTR_META_PREFIX);
	 iter != s->bucket_attrs.end(); ++iter) {
      const char *name = iter->first.c_str();
      if (strncmp(name, RGW_ATTR_META_PREFIX, PREFIX_LEN) == 0) {
				    ...
				    RGWClientIO *cio)
{
  dout(10) << "s->object=" << (!s->object.empty() ? s->object
			       : rgw_obj_key("<NULL>")) << " s->bucket="
	   << (!s->bucket_name_str.empty() ? s->bucket_name_str : "<NULL>")
	   << dendl;
