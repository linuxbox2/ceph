...
# define bendl std::endl; }
  ...
  public:
    ...
    bool shareable() {
      return false; // !shareable, will force clone_nonshareable()
    }
    ~raw_unshareable() {
      delete[] data;
    }
  ...
  buffer::ptr& buffer::ptr::clone_nonshareable() {
...
void buffer::list::rebuild_page_aligned()
{
  ...
  void buffer::list::claim(list& bl, bool clone_nonshareable)
  {
    // free my buffers
    clear();
    claim_append(bl, clone_nonshareable);
  }

  void buffer::list::claim_append(list& bl, bool clone_nonshareable)
  {
    // steal the other guy's buffers
    _len += bl._len;
    if (clone_nonshareable)
       bl.clone_nonshareable();
    _buffers.splice(_buffers.end(), bl._buffers );
    bl._len = 0;
    bl.last_p = bl.begin();
  }

  void buffer::list::claim_prepend(list& bl, bool clone_nonshareable)
  {
    // steal the other guy's buffers
    _len += bl._len;
    if (clone_nonshareable)
